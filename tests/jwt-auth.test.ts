...
describe("JWT auth", () => {
    test("supports public routes, JWT auth and role-based authorization", async () => {
        ...
        const profile = await app.handle(new Request("http://localhost/auth/profile", {
            headers: { authorization: `Bearer ${adminToken}` },
        }));
        expect(profile.status).toBe(200);
        expect(await profile.json()).toEqual({ sub: "123" });
        const forbidden = await app.handle(new Request("http://localhost/auth/admin", {
            headers: { authorization: `Bearer ${userToken}` },
        }));
        expect(forbidden.status).toBe(403);
        const admin = await app.handle(new Request("http://localhost/auth/admin", {
            headers: { authorization: `Bearer ${adminToken}` },
        }));
