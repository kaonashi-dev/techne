...
import { BusRegistry, Command, CommandBus, CommandHandler, CqrsQuery, DomainEvent, EventBus, EventHandler, InMemoryEventStore, QueryBus, QueryHandler, } from "../src/cqrs";
...
class CreateUserHandler {
    constructor(private readonly repo: UsersRepo, private readonly eventBus: EventBus) { }
    async execute(command: CreateUserCommand) {
        this.repo.items.push(command.payload.name);
        await this.eventBus.emit(new UserCreatedEvent({ name: command.payload.name }), command.payload.name);
    }
}
