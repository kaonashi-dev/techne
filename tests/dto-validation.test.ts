...
import { Dto, IsString, IsNumber, IsBoolean, IsInteger, IsEnum, IsOptional, MaxLength, Min, Schema, ValidateNested, } from "../src/schema";
enum Role {
    Admin = "admin",
    Editor = "editor"
}
...
describe("@Dto() + @Body(DtoClass) — auto schema injection", () => {
    ...
    test("returns 422 when body fails validation", async () => {
        ...
        // name too short
        const res = await app.handle(new Request("http://localhost/users-invalid", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    test("explicit route schema takes priority over @Body(Dto)", async () => {
        ...
        const res = await app.handle(new Request("http://localhost/explicit", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    test("works with IsEnum and IsInteger", async () => {
        ...
        const valid = await app.handle(new Request("http://localhost/products", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        const invalid = await app.handle(new Request("http://localhost/products", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
    ...
    test("infers typed @Body() DTO without @Dto decorator", async () => {
        ...
        const valid = await app.handle(new Request("http://localhost/profiles", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                name: "alice",
        ...
        expect(valid.status).toBe(200);
        const invalid = await app.handle(new Request("http://localhost/profiles", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                name: "alice",
    ...
    test("native DTO schemas reject unknown properties by default", async () => {
        ...
        const res = await app.handle(new Request("http://localhost/strict-profiles", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
                name: "alice",
