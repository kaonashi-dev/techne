...
describe("public API", () => {
  test("exposes Techne common and core subpaths", async () => {
    ...
    expect("Module" in common).toBe(false);
    expect("ValidationPipe" in common).toBe(false);
    ...
    expect(typeof common.OnResponse).toBe("function");
    expect("UseInterceptors" in common).toBe(false);
    expect("UsePipes" in common).toBe(false);
    ...
    expect("ContextIdFactory" in core).toBe(false);
    expect(typeof core.ModuleRef).toBe("function");
    expect("ConfigModule" in config).toBe(false);
    expect(typeof config.config).toBe("function");
    expect(typeof config.appConfig).toBe("function");
    expect(typeof config.ConfigService).toBe("function");
    expect("JwtModule" in jwt).toBe(false);
    expect(typeof jwt.jwt).toBe("function");
  ...
  test("keeps queue decorators scoped to the queue subpath", async () => {
    ...
    expect("MqModule" in mq).toBe(false);
    expect(typeof mq.mq).toBe("function");
