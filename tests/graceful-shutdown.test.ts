...
describe("Graceful shutdown", () => {
  test("zero grace period skips in-flight request counting", async () => {
    const adapter = new ElysiaAdapter({
      logger: false,
      shutdown: { gracePeriod: 0 },
    });
    ...
    const pending = app.handle(new Request("http://localhost/slow"));
    await Bun.sleep(5);
    expect(adapter.getInflightCount()).toBe(0);
    adapter.setDraining(true);
