...
import { APP_CONFIG, ConfigValidationError, InjectConfig, appConfig, defineConfig, t, type AppConfig, } from "../src/config";
...
describe("defineConfig", () => {
    ...
    test("appConfig() exposes APP_CONFIG and InjectConfig resolves it", async () => {
        const config = defineConfig({
            schema: baseSchema,
            source: {
                PORT: "4242",
        ...
        class Consumer {
            constructor(
            @InjectConfig()
            readonly direct: AppConfig<typeof baseSchema>, 
            @Inject(APP_CONFIG)
            readonly raw: AppConfig<typeof baseSchema>) { }
        }
