...
describe("CLI project generator", () => {
    ...
    test("creates a complete starter project", async () => {
        ...
        const appController = await fs.readFile(path.join(projectDir, "src", "app.controller.ts"), "utf8");
    ...
    test("fails when target directory is not empty", async () => {
        ...
        await expect(createProject("existing-project")).rejects.toThrow(/already exists and is not empty/);
    });
});
