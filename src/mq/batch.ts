...
export class BatchBuilder {
  ...
  async dispatch(): Promise<BatchHandle> {
    ...
    const callbacks: BatchCallbacks = {
      // oxlint-disable-next-line no-thenable -- `then` is a data property key, not a Promise terminator
    ...
    // If the batch is empty, fire callbacks immediately (total === 0, done immediately).
    if (total === 0) {
 ...
 *
 * `failed` must be passed in because the store entry is deleted by cleanup.
 */
